...
|-------------|------------|
| <img src="https://danilw.github.io/GLSL-howto/vulkan_sh_launcher/images_bugs/7cS3R1_1.png" width="650" height="auto" /> | [BUG Nvidia sin hash vs iTime](https://www.shadertoy.com/view/7cS3R1) - another sin-hash bug example.  Read blog - [**Hash Noise stability in GPU Shaders**](https://arugl.medium.com/hash-noise-in-gpu-shaders-210188ac3a3e).|
...
| <img src="https://danilw.github.io/GLSL-howto/vulkan_sh_launcher/images_bugs/t3BBWK.png" width="650" height="auto" /> | [`BUG -1%2 -> >0`](https://www.shadertoy.com/view/t3BBWK) it known behavior I think but I dont remeber if I made post about it - `-1%2==negative` - this behavior completely different for each of OpenGL DX11 and Vulkan. in Vulkan CPU-pre-compiler(rght side on screenshot) return positive when CPU-comp on DX11 and OpenGL return negative, when in GPU - DX11 and Vulkan is different to OpenGL.|
| <img src="https://danilw.github.io/GLSL-howto/vulkan_sh_launcher/images_bugs/w32fDy.png" width="650" height="auto" /> | [BUG normalize(rd)*rot -> rd.x>1.](https://www.shadertoy.com/view/w32fDy) - `normalize(rd); -> (rotX*rotY)*rd; -> length(rd)>1.0` - length of normalized vector after rotation is `>1.0` and then I use `sqrt(1. - rd.x * rd.x);` it return UB - look next screenshot. This is same as in "not a bug" section below - **BUG normalize bugs** - technically it is not a bug - but had to spend time to debug it.|
| <img src="https://danilw.github.io/GLSL-howto/vulkan_sh_launcher/images_bugs/w3SfWm_wX2BR1.png" width="650" height="auto" /> | real case of `sqrt(1. - rd.x * rd.x);` above - where rd.x is normalized but after rotation - white dots on middle of screenshots. *Bug found in [w3SfWm](https://www.shadertoy.com/view/w3SfWm) [wX2BR1](https://www.shadertoy.com/view/wX2BR1)*.|
| <img src="https://danilw.github.io/GLSL-howto/vulkan_sh_launcher/images_bugs/wcSyRV.jpg" width="650" height="auto" /> | [BUG Nvidia OpenGL compiler -log2](https://www.shadertoy.com/view/wcSyRV) - Nvidia OpenGL compiler bug `return (-k * log2(r));` bugged and not equal to `return -1.*(k * log2(r));` [Nvidia](https://forums.developer.nvidia.com/t/nvidia-opengl-compiler-bug-k-log2-r-bugged/343861)|
...
| **hash instability** | *just for visibility* - https://github.com/danilw/GPU-sin-hash-stability |
...
[Old GLES2 depth write bug on AMD](https://community.amd.com/t5/opengl-vulkan/amd-gles2-depth-write-broken/m-p/279353) - 2025 amd deleted their community forum - [old link to godot bugreport with visuals (not godot bug)](https://github.com/godotengine/godot/issues/36812) - still broken, I think it another wontfix because no one care about OpenGL. (*Note* - by clicking this link you may trigger interesting web-bug, look below "*Interesting amd forums bug*" - just press enter in address text in webbrowser if you get 403)
___
...
| <img src="https://danilw.github.io/GLSL-howto/vulkan_sh_launcher/images_bugs/tXcXDl_720.png" width="650" height="auto" /> | [GPU bug-pattern-art (Nvidia)](https://www.shadertoy.com/view/tXcXDl) - Just "Hello" word build from bug-float-pattern-lines it suppose to work(display intended as on screenshot) only on Nvidia GPUs. |
